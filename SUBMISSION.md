...
| 4 | **Innovation / Bonus** | 15% | Astro-specific augmentations (cosmic-ray sim, vignetting, Poisson noise) `src/dataset.py`; TTA (6× aug) in `src/evaluate.py`; bonus features `src/bonus.py`; real-first data pipeline `src/prepare_data.py` with honest procedural fallback (`data/processed/DATA_MANIFEST.json`); Streamlit demo `app/app.py` |
...
- **Demo video** (Deliverable 5): [`docs/presentation/demo.mp4`](docs/presentation/demo.mp4) — ~76 s (75.8 s), 1280×720, H.264. Walks through architecture → metrics → confusion matrix → per-class F1 → confidence distribution → all 15 Grad-CAM samples → summary grid. Script: [`docs/presentation/Demo_Video_Script.md`](docs/presentation/Demo_Video_Script.md).
- Real data sources: Galaxy10 DECaLS survey imagery (via astroNN) for Spiral + Elliptical; **NASA Image Library** (images.nasa.gov, no API key — Hubble/Spitzer/JPL) for Nebula / Star Cluster / Planetary, retrieved by keyword with per-class purity filtering (`src/download_archives.py`). Kaggle deep-space images (`fedesoriano/deep-space-images`, `brsdincer/planetary-solar-system-objects`) and a clearly-labelled procedural fallback remain as fallbacks only. Galaxy Zoo, SDSS, ESA Hubble, NASA Hubble, NASA PDS, and ESA PSA are the public archives named in `PROBLEM_STATEMENT.md`; Galaxy10 is the ML-ready derivative of Galaxy Zoo + SDSS/DECaLS, and NASA Image Library is the searchable API branch of NASA's Hubble/JPL imagery. ESA Hubble, NASA PDS, and ESA PSA are referenced but not auto-downloaded in this pipeline.
